...
2. How should `crane apply` behave when cluster-scoped resources already exist on the target with conflicting definitions — skip or fail? (Merge/reconcile is excluded per Non-Goals.)
    * **Decision:** Fail by default and report the conflict. Since `crane apply` delegates to `kubectl apply` which uses declarative semantics (update-if-exists), the practical behavior is that existing resources get updated. Add a `--dry-run` recommendation in documentation so users can preview before applying.
