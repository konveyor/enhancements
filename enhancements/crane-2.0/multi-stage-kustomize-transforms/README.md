...
1. **Migration descriptor file vs. CLI positional arguments + conventions (dev preferred):** Should Crane require a declarative descriptor file that specifies which transformers run and in what order, or is the current approach sufficient — CLI commands with positional arguments (stage or plugin names) combined with the convention-based directory structure (`10_PluginName/`, `20_PluginName/`, `50_CustomChange/`)? The current approach keeps configuration minimal and co-located with the output. A future migration pipeline abstraction may introduce a migration plan file (including these descriptors) naturally, but it is unclear whether the transform descriptor file is needed at the single-tool level.
    * **Decision:** Both. Keep CLI positional arguments + conventions as the default for simple cases. Add a transform descriptor file (tracked in [crane#370](https://github.com/migtools/crane/issues/370)) for repeatable multi-stage workflows. The descriptor file is optional and complements the CLI approach rather than replacing it.
...
3. **Enable helm templating within Kustomize.** While Kustomize is the standard for patching manifests, manually writing complex, conditional custom transformations (e.g. using components) might be difficult. One workaround is using AI (like Claude Code) to write custom plugins per use case. However, a simpler approach might be to allow Kustomize to evaluate Helm templates directly via `kustomization.yaml`. Note, those template files will not be generated by crane, so it will require users having experience with helm.
    * **Decision:** Defer to a future enhancement. Kustomize components and custom transform stages cover most use cases today. Helm integration adds complexity and requires users to have Helm experience. If needed, users can use Kustomize's built-in `helmCharts` generator in their custom stages without crane changes.
...
* **Debugging:** If a resource (like a Route or Deployment) is incorrectly modified, I can easily isolate the behavior of a single plugin by inspecting its specific patch folder or by re-executing just that specific stage using the `crane transform <plugin_or_stage_name>` command.
...
**`crane transform` — interface changes:**

| Argument/Flag | Purpose |
|------|---------|
| `[<plugin_or_stage_name>...]` | Transform only selected stages (e.g., `10_KubernetesPlugin 20_OpenShiftPlugin`). Accepts plugin names or stage directory names. Defaults to all stages if empty. |
...
* Supports same stage-selection arguments as `transform`
...
Stages are discovered by convention: Crane scans the transform directory for subdirectories matching `<number>_<pluginName>`. By default, `crane transform` creates only `10_KubernetesPlugin`. Additional plugin stages are created explicitly (via `crane transform [<plugin_or_stage_name>...]`, or via descriptor file — see [crane#370](https://github.com/migtools/crane/issues/370)), with priorities starting at 20 and gaps of 10 to allow user-defined custom stages.

Available plugins for transform stages creation can be listed with `crane transform list-plugins` command or discovered via shell tab completion.

#### One Plugin Per Stage

Each stage executes exactly one plugin. This isolation ensures that when a transformation produces unexpected results, the responsible plugin can be identified immediately by inspecting the corresponding stage directory. Users who need multiple plugins chain multiple plugin or custom stage names to the `crane transform` command. For recurring multi-plugin workflows, it is possible to define all stages in a single configuration.
...
**Stage state drift.** Plugin-backed stages (e.g., `10_KubernetesPlugin`) can be safely regenerated by `crane transform`. User-modified custom stages require `--force` to overwrite. (See Open Question 2 for full policy discussion.)
